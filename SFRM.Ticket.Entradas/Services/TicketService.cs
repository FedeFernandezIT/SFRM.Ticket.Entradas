            ...
            try
            {
                using (var db = new TicketContext(TicketServer, TicketDatabase))
                {
                    using (var trans = db.Database.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
                    {
                        try
                        {
                            var sql = @"SELECT t.tickets_id AS TicketNro, s.season_name AS Temporada, e.event_name AS Evento, e.event_date AS Fecha, z.zone_name AS Zona, t.ticket_row AS Fila, t.ticket_seat_id AS Asiento, t.ticket_price AS Precio, tt.ticket_type_name AS Tipo, t.ticket_qr AS CodigoQr FROM tickets AS t " +
                            ...
                            var entradas = db.Database.SqlQuery<Entrada>(sql,
